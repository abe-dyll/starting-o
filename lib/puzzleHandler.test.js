...
const NAMES_BY_POSITION = {
  QB: ['Brock Osweiler', 'Peyton Manning'],
  RB: ['Knowshon Moreno', 'Montee Ball'],
  WR: ['Demaryius Thomas', 'Eric Decker', 'Wes Welker'],
  TE: ['Julius Thomas'],
};
...
test('buildInitialPuzzleResponse returns round 1 with no hints and a valid token', () => {
  const response = buildInitialPuzzleResponse({
    ...
    secret: 'test-secret',
    namesByPosition: NAMES_BY_POSITION,
  });
...
test('buildInitialPuzzleResponse passes through the injected namesByPosition pool unchanged', () => {
  const response = buildInitialPuzzleResponse({
    puzzles: PUZZLES,
    now: () => new Date('2026-01-01T00:00:00Z'),
    secret: 'test-secret',
  ...
  assert.deepEqual(response.namesByPosition, NAMES_BY_POSITION);
});
