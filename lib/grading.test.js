...
test('a correct round-1 guess scores 200 and locks the slot', () => {
  ...
  assert.deepEqual(result.results.QB, {
    correct: true,
    name: 'Peyton Manning',
    pointsAwarded: 200,
    hints: { jersey: 18, height: "6'5\"", weight: 230, stat: '450 attempts' },
  });
...
test('a correct guess for WR1 typed into the WR2 box still counts, crediting the matched slot', () => {
  const result = gradeRound({
    puzzle: PUZZLE,
    tokenState: freshTokenState(1),
    guesses: { QB: '', RB: '', WR1: '', WR2: 'Demaryius Thomas', TE: '' },
  });

  assert.equal(result.results.WR1.correct, true);
  assert.equal(result.results.WR1.name, 'Demaryius Thomas');
  assert.equal(result.results.WR2.correct, false);
  assert.equal(result.newTokenState.solved.WR1, 1);
  assert.equal(result.newTokenState.solved.WR2, null);
});

test('typing the same WR name in both boxes only credits it once', () => {
  const result = gradeRound({
    puzzle: PUZZLE,
    tokenState: freshTokenState(1),
    guesses: { QB: '', RB: '', WR1: 'Demaryius Thomas', WR2: 'Demaryius Thomas', TE: '' },
  });
